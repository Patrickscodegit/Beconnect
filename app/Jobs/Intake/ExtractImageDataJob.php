...
class ExtractImageDataJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    public $timeout = 600; // 10 minutes for image extraction (may need OCR)
    ...
    {
        Log::info('ExtractImageDataJob: Starting image data extraction', [
        ...
        try {
            ...
            // Extract image data using specialized strategy
            ...
            if ($extractionResult->success) {
                ...
                Log::info('ExtractImageDataJob: Image extraction completed successfully', [
                    'intake_id' => $this->intake->id,
                    'file_id' => $this->file->id,
                ...
                throw new \Exception("Image extraction failed: {$extractionResult->error}");
            }

        } catch (\Exception $e) {
            Log::error('ExtractImageDataJob: Failed to extract image data', [
                'intake_id' => $this->intake->id,
                'file_id' => $this->file->id,
                'error' => $e->getMessage(),
    ...
    private function createDocumentFromIntakeFile(): Document
    {
        ...
        if ($existingDocument) {
            Log::info('ExtractImageDataJob: Using existing document', [
                'intake_id' => $this->intake->id,
        ...
        Log::info('ExtractImageDataJob: Created new document', [
            'intake_id' => $this->intake->id,
            'file_id' => $this->file->id,
            'document_id' => $document->id
        ]);
    ...
    private function updateProcessedDocumentsCount(): void
    {
        ...
        Log::debug('ExtractImageDataJob: Updated processed documents count', [
            'intake_id' => $this->intake->id,
    ...
    {
        Log::error('ExtractImageDataJob failed permanently', [
            'intake_id' => $this->intake->id,
            'file_id' => $this->file->id,
            'filename' => $this->file->filename,
