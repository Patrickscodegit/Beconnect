...
class GrimaldiPurchaseRatesOverview extends Page
{
    ...
    {
        // #region agent log
        @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'F', 'location' => 'GrimaldiPurchaseRatesOverview.php:678', 'message' => 'applyPortDates entry', 'data' => ['portCode' => $portCode, 'portDates' => $this->portDates[$portCode] ?? []], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
        // #endregion
        ...
        try {
            ...
            // #region agent log
            @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'F', 'location' => 'GrimaldiPurchaseRatesOverview.php:690', 'message' => 'Date values to apply', 'data' => ['portCode' => $portCode, 'updateDate' => $updateDate, 'validityDate' => $validityDate], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
            // #endregion
            ...
            CarrierPurchaseTariff::query()
                ...
                ->chunkById(200, function ($tariffs) use ($updateDate, $validityDate, $syncService, &$updatedArticleCodes) {
                    // #region agent log
                    @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'F', 'location' => 'GrimaldiPurchaseRatesOverview.php:712', 'message' => 'Chunk processing start', 'data' => ['tariffCount' => $tariffs->count()], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
                    // #endregion
                    // No transaction
                    foreach ($tariffs as $tariff) {
                        if ($updateDate !== null) {
                        ...
                        try {
                            ...
                            // #region agent log
                            @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'A', 'location' => 'GrimaldiPurchaseRatesOverview.php:726', 'message' => 'Tariff save failed', 'data' => ['tariffId' => $tariff->id, 'error' => $e->getMessage()], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
                            // #endregion
                            throw $e;
                        }
                        ...
                        // #region agent log
                        @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'B', 'location' => 'GrimaldiPurchaseRatesOverview.php:738', 'message' => 'After sync call', 'data' => ['tariffId' => $tariff->id], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
                        // #endregion
