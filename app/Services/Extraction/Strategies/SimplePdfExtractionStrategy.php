...
class SimplePdfExtractionStrategy implements ExtractionStrategy
{
    ...
    private function extractContactInfo(string $text, array &$extractedData): void
    {
        ...
        if ($consigneeStart !== false && $notifyStart !== false) {
            ...
            // Updated regex to handle both "No. X" and "Road X" formats
        ...
        if ($notifyStart !== false && $invoiceStart !== false) {
            ...
            if (preg_match('/^([A-Za-z\s\.&,]+?)\s+(?:No\.\s+(\d+)\s+([A-Za-z0-9\s,()]+?)\s+([A-Za-z\s,]+?)|Road\s+(\d+)\s+([A-Za-z0-9\s,()]+?)\s+([A-Za-z\s,]+?))\s+([A-Za-z0-9@\.]+)\s+(\+\d+\s+\d+)/', $notifySection, $matches)) {
                ...
                if (preg_match('/Notify\s+([A-Za-z\s\.&,]+?)\s+(?:No\.\s+\d+\s+([A-Za-z0-9\s,()]+?)\s+([A-Za-z\s,]+?)|Road\s+\d+\s+([A-Za-z0-9\s,()]+?)\s+([A-Za-z\s,]+?))\s+([A-Za-z0-9@\.]+)\s+(\+\d+\s+\d+)/', $text, $matches)) {
                    ...
                    if (!empty($matches[2])) {
                        // "No. X" pattern
                        $extractedData['notify']['address'] = 'No. ' . trim($matches[2]) . ' ' . trim($matches[3]);
                        $extractedData['notify']['email'] = trim($matches[4]);
                        $extractedData['notify']['phone'] = trim($matches[5]);
                    } else {
                        // "Road X" pattern
                        $extractedData['notify']['address'] = 'Road ' . trim($matches[4]) . ' ' . trim($matches[5]);
                        $extractedData['notify']['email'] = trim($matches[6]);
                        $extractedData['notify']['phone'] = trim($matches[7]);
                    }
                }
            }
        }
