...
class TariffDateSyncService
{
    ...
    public function syncTariffDatesToArticle(CarrierPurchaseTariff $tariff): void
    {
        ...
        if (!$mapping || !$mapping->article) {
            // #region agent log
            @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'C', 'location' => 'TariffDateSyncService.php:22', 'message' => 'Early return - no mapping/article', 'data' => ['tariffId' => $tariff->id], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
            // #endregion
            return;
        }
        ...
        // #region agent log
        @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'D', 'location' => 'TariffDateSyncService.php:36', 'message' => 'Date change detection', 'data' => ['articleId' => $article->id, 'updateDateChanged' => $updateDateChanged, 'validityDateChanged' => $validityDateChanged], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
        // #endregion
        
        // Only update if something changed
        if (!$updateDateChanged && !$validityDateChanged) {
            // #region agent log
            @file_put_contents(storage_path('logs/debug.log'), json_encode(['sessionId' => 'debug-session', 'runId' => 'run1', 'hypothesisId' => 'D', 'location' => 'TariffDateSyncService.php:40', 'message' => 'No-op: dates unchanged', 'data' => ['articleId' => $article->id], 'timestamp' => time() * 1000]) . "\n", FILE_APPEND);
            // #endregion
